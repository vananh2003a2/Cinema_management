...
<body class="bg-dark">
	<div class="choosecombo-header text-center text-white h3" style="margin-top: 105px;">
		<a href="pickchair" class="choosecombo-header-back"><svg
					...
					<c:forEach items="${lstcombo}" var="combo">
						<div class=" col-3">
							<div class="card bg-white text-black">
								<img src="https://starlight.vn/Content/img/combo-doi.jpg"
									class="card-img-top" alt="">
								<div class="card-body text-black">
									<p class="comboId card-text text-center" style="display: none;">${combo.getIdcombo() }</p>
									<h5 class="comboName card-title text-center ">${combo.getComboname() }</h5>
									<p class="card-text text-center">${combo.getComboname() }</p>
									<p
										class="comboPrice card-text text-color-main-100 text-center mb-4"
									...
									<div
										class="choosecombo-content-quality d-flex justify-content-center">
		...
		<div class="row choosecombo-footer-info">
			...
			<div class="col-2 d-flex align-items-center px-0">
				<form id="confirmationForm" action="xacnhanthongtin" method="POST">
					...
					<input type="hidden" name="idRoom" value="${idRoom }"> <input
						type="hidden" name="idMovie" value="${idMovie }"> <input
						type="hidden" name="idTimeRoom" value="${idMovieTimeRoom }"> 
					<!-- Thông tin ghế đã chọn -->
					<input type="hidden" name="idChairTime" value="${idChairTime }"> 
					<input type="hidden" name="selectedChairs" value="${selected_idchairs }"> 
					...
					<input type="hidden" id="comboQuantity" name="comboQuantity" value="0">
					<input type="hidden" id="comboPrice" name="comboPrice" value="0">
					<!-- Các trường khác cần gửi đi -->
					<button style="width: 190px;" type="submit"
						class="btn choosecombo-footer-info-btn fw-bold text-color-main-100 text-white">Tiếp
						theo</button>
				</form>
    ...
    const comboIdElements = document.querySelectorAll('.comboId');
    ...
    plusBtns.forEach(function(btn, index) {
        btn.addEventListener('click', function(event) {
            ...
            let comboIdText = comboIdElements[index].textContent.trim(); // Lấy giá trị idcombo từ phần tử comboId tương ứng
            updateSelectedCombo(index, quantity, comboIdText);
        });
    });
    ...
    function updateSelectedCombo(index, quantity) {
        let comboIdText = document.querySelectorAll('.comboId')[index].textContent.trim(); // Lấy id của combo
        ...
        if (quantity === 0) {
            ...
            selectedCombos[index] = {
                ...
                name: comboNameText,
                quantity: quantity,
                price: parseFloat(comboPriceText) * quantity
            };
    ...
    function updateFooterInfo() {
        ...
        document.getElementById('idCombo').value = Object.values(selectedCombos).map(combo => combo.id).join(','); // Lấy danh sách id combo
